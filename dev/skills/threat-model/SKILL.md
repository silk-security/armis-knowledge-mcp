---
name: threat-model-dev
description: "Generate a threat model for a target application using local code-intelligence (the bundled armis-code-mcp server). Produces armis-appsec-v2-results/<repo>/THREAT_MODEL.md with an 8-section, stable-ID schema (T1, T2, … threats; M1, M2, … mitigations) that downstream appsec skills can cite. Use when the user asks to create a threat model, security analysis, or risk assessment of a codebase. Triggers: /threat-model-dev, threat model this repo, security risk assessment, attack surface analysis."
---

# /threat-model-dev — Threat Model Generator
...
use the `mcp__armis_code_mcp_dev__*` tools. The binary indexes the repo into a
...
1. **Index** the target if it is not already indexed:
   ```
   mcp__armis_code_mcp_dev__index_repository(repo_path=<target-dir>)
   ```
...
4. **Collect facts.** Run these queries against the graph; do not skip them —
   ...
   - `mcp__armis_code_mcp_dev__get_architecture(project=<project_id>, aspects=["all"])`
   - `mcp__armis_code_mcp_dev__search_graph(project=<project_id>, label="Route")`
   - `mcp__armis_code_mcp_dev__search_graph(project=<project_id>, label="Function")`
   - `mcp__armis_code_mcp_dev__search_graph(project=<project_id>, label="Class")`
   - `mcp__armis_code_mcp_dev__search_code(project=<project_id>, pattern="valid|sanitiz|auth|check|verify|hash|encrypt|token|csrf|password", regex=true, mode="full")`
   - `mcp__armis_code_mcp_dev__search_code(project=<project_id>, pattern="secret|key|password|credential|token|api_key|env|config", regex=true, mode="compact")`
...
- **Ground in the MCP data.** The route list, function inventory, and data model
  must come from `mcp__armis_code_mcp_dev__*` queries — not assumptions.
