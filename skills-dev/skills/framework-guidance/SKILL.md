...
Fetch Armis-specific guidance for a web framework from the **dev** Knowledge backend.

```bash
source "$CLAUDE_PLUGIN_ROOT/lib/armis-knowledge.sh"
TENANT_ID=$(ak_tenant_id)
NAME="${ARGUMENTS:?usage: /framework-guidance-dev <framework>  (e.g. django, flask, fastapi, express, rails, spring)}"
...
3. Cite: "Applied Armis Knowledge dev framework-guidance: <name>."
