---
name: cwe-fix-report-dev
description: "Produce a structured before/after report after a CWE-fix run against the Armis Knowledge DEV environment (shell-skills variant). Uses git diff + a scanner-provided baseline to build the fix table, cites the org standards each fix used, lists residuals, and optionally opens a PR. Use after finishing a batch of CWE fixes when the user asks for a report / diff / PR body. Triggers: /cwe-fix-report-dev, generate cwe fix report, report the fixes, cwe report, open a pr for the fixes."
allowed-tools:
  - Bash(curl *)
  - Bash(jq *)
  - Bash(source *)
...
For every CWE closed between initial and final, look up the org's remediation doc for that CWE via the dev Knowledge backend. The `cwe-fix-dev` skill in this plugin already exposes the exact API call — reuse its pattern rather than re-implementing here (source the plugin's armis-knowledge shell helper library and call the content-pack GET with `pack=cwes` and `name=CWE-<n>`).
