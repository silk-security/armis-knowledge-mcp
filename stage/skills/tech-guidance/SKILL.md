...
- The user runs `/tech-guidance-stage python` → call `mcp__armis_knowledge_stage__get_technology_guidance("python")`.
...
- Auth errors → check `ARMIS_CLIENT_ID` / `ARMIS_CLIENT_SECRET` are set; rotate at `knowledge.moose-stg.armis.com/settings/integrations` if needed.
