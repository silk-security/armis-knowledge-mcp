...
- The user runs `/tech-guidance python` → call `mcp__armis_knowledge_stage__get_technology_guidance("python")`.
...
2. Mention you applied "Armis Knowledge (stage) tech-guidance: <name>" so the user can audit.
...
- Auth errors → check `ARMIS_CLIENT_ID` / `ARMIS_CLIENT_SECRET` / `ARMIS_KNOWLEDGE_TENANT_SLUG` are set; rotate at `knowledge.moose-stg.armis.com/settings/integrations` if needed.
