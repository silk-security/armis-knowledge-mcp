...
- If the server returns an auth error, the user's `ARMIS_CLIENT_ID` / `ARMIS_CLIENT_SECRET` / `ARMIS_TENANT_SLUG` env vars are missing or invalid — point them at `knowledge.moose-stg.armis.com/settings/integrations` to issue / rotate credentials.
