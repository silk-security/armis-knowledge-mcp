...
Fetch Armis-specific remediation for a CWE from the **dev** Knowledge backend.

```bash
source "$CLAUDE_PLUGIN_ROOT/lib/armis-knowledge.sh"
TENANT_ID=$(ak_tenant_id)
CWE="${ARGUMENTS:?usage: /cwe-fix-dev CWE-89}"
...
3. Cite the doc in your response: "Applied Armis Knowledge dev CWE-89 guidance."
