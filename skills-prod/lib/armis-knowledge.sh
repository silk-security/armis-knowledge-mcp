# Shared helpers for the armis-knowledge-skills (prod) plugin.
#
...
#
# Optional env:
#   ARMIS_KNOWLEDGE_API   override backend URL (default: prod)
#
# The client_secret lives in the OS keychain, NOT a shell rc:
#   macOS:  security add-generic-password -s armis-knowledge-prod -a "$ARMIS_KNOWLEDGE_CLIENT_ID" -w
#   Linux:  secret-tool store --label='Armis Knowledge prod' service armis-knowledge-prod account "$ARMIS_KNOWLEDGE_CLIENT_ID"

set -euo pipefail

ARMIS_KNOWLEDGE_ENV="${ARMIS_KNOWLEDGE_ENV:-prod}"
ARMIS_KNOWLEDGE_API="${ARMIS_KNOWLEDGE_API:-https://knowledge-api.moose.armis.com}"
